// replace this array with a series of randomly created string literals and arrays
const text = [
...
// API or otherwise populate this const with weeb-words
const weeb = [];
...
form.addEventListener('submit', function(e){
    ...
    function getRandomLorem() {
        const innerRandom = Math.floor(Math.random() * text.length)
        return `<p class="result">${text[innerRandom]}</p>`;
    }
    result.innerHTML = Array.from({length: value}, () => getRandomLorem());
    } 
})
...
// if(isNaN(value) || value <= 0 || value > 9) {
        ...
        // } else{
                ...
                // return `<p class="result">${text[random]}</p>` // Math.floor(Math.random() * tempText.length)
                // }).join("");
